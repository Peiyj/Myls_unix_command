 ...
 * Return Value: -1 if the first time is older, 0 if they are the same
 * or +1 if the first time is newer.
 * Registers Used: 
...
else_if:
    ...
    ldr r1, [fp, P2_OFFSET]      @ load the 2nd pointet
    ldr r1, [r1]                 @ load the struct offset
    ldr r2, =offset_time         @ load the time offset
    ldr r2, [r2]                 @ load the content
    ldr r1, [r1, r2]             @ get 2nd name into r1
    ...
    beq else                     @ branch if numbers are equal
    @ return 1                 
